...
const Gallery = ({ images, title, subtitle, tag, className }: GalleryProps) => {
  ...
  const handlePreviousImage = useCallback(() => {
    ...
    setExpandedImage(expandedImage === 0 ? images.length - 1 : expandedImage - 1);
  }, [expandedImage, images.length]);

  const handleNextImage = useCallback(() => {
    if (expandedImage === null) return;
    setExpandedImage(expandedImage === images.length - 1 ? 0 : expandedImage + 1);
  }, [expandedImage, images.length]);
